...
for n = 1:(N)
    ...
    Y(n+1) = Y(n)*(1 + (mu)*h + sigma*(W(n+1)-W(n))+0.5*sigma^2*((W(n+1)-W(n))^2-h));
end
...
plot(x,X,'black*',x,Y,'b--',x,Actual,'r--', "LineWidth", 2)
...
set(gca,'fontsize',30)
legend('EM Solution','Milstein Solution','Actual Solution','Location','southeast')
