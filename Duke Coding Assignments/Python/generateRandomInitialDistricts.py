...
def compressCentroid(A,B):
    #Takes in two nodes and compresses thier centroids into a single centroid
...
while i < numMaps:
    ...
    createFile = outFilePrefix.split('/')
    currentLine = ""
    for line in createFile:
        currentLine += line + "/"
        if not os.path.exists(currentLine):
            os.makedirs(currentLine)

    distFileOut = open(os.path.join(outFilePrefix, "districtingMap"+str(i)+".txt"),"w")
    for d in range(len(Groups)):
        currentDistrict = Groups[d]
        print "district ", str(d), " population: ", currentDistrict.population
        precinctsInDistrict = currentDistrict.precincts
        for p in precinctsInDistrict:
            distFileOut.write(str(p) + '\t' + str(d) +  "\n")
    distFileOut.close()
    i += 1
